...
//SDFileSystem sd(MICROBIT_PIN_P15, MICROBIT_PIN_P14, MICROBIT_PIN_P13, MICROBIT_PIN_P8, "sd"); // MOSI, MISO, SCLK, SSEL
...
void LogData(FILE *fp)
{
    ...
    fprintf(fp,"%02d,%06u,%03u,%06u,%03d,%03d,%05d,%05d,%05d,%03d,%03d\n", satelliteId,IR,UV,V,Temp,Heading,X,Y,Z,Pitch,Roll);
...
int main() { 
    ...
    MicroBitMultiButton buttonAB = MicroBitMultiButton(MICROBIT_ID_BUTTON_A, MICROBIT_ID_BUTTON_B, MICROBIT_ID_BUTTON_AB); 
    if (!SI1145begin())
    {
        uBit.display.scrollAsync("No sat kit");
        return 1;
    }
    ...
    while((p = readdir(d)) != NULL) {         // Print the names of the files in the local file system
       ...
       fileincrement++;
       printf("FileIncrement %d", fileincrement);
    }
    closedir(d);
    ...
    uBit.display.scrollAsync("Closed file",100);
    return 1;
}
